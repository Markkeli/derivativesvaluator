...
NDCalculator::NDCalculator()
{

}

double NDCalculator::inverseNormalDistribution(double probability, double min, double max)
{
    double x = 0;
    double a = 0;
    double b = 1;
...
double NDCalculator::NormalDistribution(double x, double mean, double sigma)
{
    ...
    const double min = 4.94065645841247E-324;
    double z = arg;
    double y = std::abs(z);
    ...
    if ( y <= thrsh )
    {
        ...
        //
        xsq = zero;
        if (y > 4.94065645841247E-324) xsq = z * z;
        xnum = a[4] * xsq;
        xden = xsq;
        for (i = 0; i < 3; i++)
        {
...
double NDCalculator::NormInv(double probability)
{
    ...
    if (probability < pLow)
    {
        ...
        q = std::sqrt(-2 * std::log(probability));
    ...
    else if (probability < 1)
    {
        ...
        q =std::sqrt(-2 * std::log(1 - probability));
        result = -(((((c1 * q + c2) * q + c3) * q + c4) * q + c5) * q + c6) / ((((d1 * q + d2) * q + d3) * q + d4) * q + 1);
    }
...
double NDCalculator::NormInv(double probability, double mean, double sigma)
{
